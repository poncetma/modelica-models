...
Inputs: Evaporator wall temperature, integrated heat flux out of condenser
Outputs: Integrated heat flux through evaporator wall, condenser wall temperature 
*/
model HeatPipe_ZuoFaghri_MSLcomponents
  import Modelica.Constants.pi;
  //----------------------------
  // Geometry (KRUSTY-like)
  //----------------------------
  parameter Integer N_HPs = 8;
...
// cp = 1658.2 - 0.8479*T + 4.4541e-4*T^2 - 2.9926e6 / T^2  (units J/kg-K)
  ...
  //Modelica.Blocks.Interfaces.RealInput Q_evap_interface = 5000/8;
  parameter Real Q_cond_nominal = -1.*5000.0/N_HPs;
  input Real Q_cond_input; //default should be -5000/8;
  Real Q_cond_current;
  Modelica.Blocks.Interfaces.RealInput T_hot_input; 
  parameter Real T_inf = 300.0; //arbitrary
  parameter Real T_cond_nominal = 860.8254751449988;
  ...
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor M1(C = C1, T(start = 1073.15, fixed = false, displayUnit = "degC"), der_T(start = 0, fixed = true)) annotation(
    Placement(transformation(origin = {-90, 8}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor M2(C = C2, T(start = 1073.15, fixed = false), der_T(start = 0, fixed = true)) annotation(
    Placement(transformation(origin = {-34, 8}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor M3(C = C3, T(start = 873.15, fixed = false), der_T(start = 0, fixed = true)) annotation(
    Placement(transformation(origin = {18, 8}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor M4(C = C4, T(start = 873.15, fixed = false), der_T(start = 0, fixed = true)) annotation(
    Placement(transformation(origin = {70, 6}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor M5(C = C5, T(start = 973.15, fixed = false), der_T(start = 0, fixed = true)) annotation(
    Placement(transformation(origin = {-6, 50}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor M6(C = C6, T(start = 973.15, fixed = false), der_T(start = 0, fixed = true)) annotation(
  ...
  Modelica.Thermal.HeatTransfer.Sources.PrescribedTemperature MonolithWallTemp annotation(
    Placement(transformation(origin = {-140, -14}, extent = {{-10, -10}, {10, 10}})));
equation 
  Q_cond_current = -1.*HTC*(T_cond - T_inf); 
  /*
  if Q_cond_input > 1e-6 then
    Q_cond_current = Q_cond_input;
  else //in case there is no driven Q_cond flow, mimic the action of a PCS. 
    //Q_cond_current = Q_cond_nominal; 
    Q_cond_current = -1.*HTC*(T_cond - T_inf); 
  end if;
  */
  ...
  T_cond = M4.T;
  Q_hot = R1_1.Q_flow;
  Q_hot_allHPs = Q_hot*N_HPs; //manually adjust for the number of heat pipes
...
//R4_2.Q_flow; //Get the Qflow out through the cold side.
  ...
  connect(MonolithWallTemp.port, R1_1.port_a) annotation(
    Line(points = {{-130, -14}, {-114, -14}}, color = {191, 0, 0}));
...
end HeatPipe_ZuoFaghri_MSLcomponents;