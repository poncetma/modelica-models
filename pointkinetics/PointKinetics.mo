...
//parameter Real betas[6] = {2.30626E-04, 1.18771E-03, 1.15321E-03, 3.24697E-03, 9.60698E-04, 3.38222E-04} "groupwise effective delayed neutron fraction, Serpent output (MOOSE VTB)"; 
//parameter Real Beta = sum(betas) "total effective delayed neutron fraction";
...
//parameter Real lambdas[6] = {0.01249, 0.0318, 0.1094, 0.3173, 1.353, 8.665} "decay consts [s^-1]";
//parameter Real lambdas[6] = {1.24905E-02, 3.18031E-02, 1.09442E-01, 3.17298E-01, 1.35293E+00, 8.66546E+00 } "decay consts, Serpent [s^-1]";
...
//parameter Real Lambda = 1/(nu_bar*n_speed*Sigma_f) "Definition as per power formulation";
//parameter Real Lambda = 5.316e-6 "prompt mean generation time, Serpent [s]";
parameter Real Lambda = 5.5e-8; //seems very low, but set to match the Wang/Jiang paper. Little impact on gradual transients.
//parameter Real Lambda = 3.5e-5; //avg neutron generation time, as per Poston.
...
/*
if alpha_Tf_input > 1e-6 then
  alpha_Tf = alpha_Tf_input;
else
  alpha_Tf = alpha_Tf_default;
end if;
*/
