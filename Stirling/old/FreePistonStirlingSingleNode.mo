model FreePistonStirlingSingleNode
  ...
  parameter Real k_d = 200;
  parameter Real B_elec = 10.0;
  ...
  // Gas charge
  parameter Real p_ref = 3.52e6; 
  parameter Real T_guess = 0.5*(T_hot + T_cold); //700
  parameter Real Vtot0 = V_e0 + V_c0 + V_reg + V_dead;
  parameter Real m_gas = p_ref*Vtot0/(R_He*T_guess);  

  // States
  Real x_p(start=0), v_p(start=0.001); //Give a slight perturbation here
  Real x_d(start=0.0), v_d(start=0);
...
equation
  ...
  V_c = max(V_c0 - A_d*x_d + A_p*x_p, 1e-15);
  V_total = V_e + V_c + V_reg + V_dead;
  Vdot = der(V_total); //A_p*v_p;
...
end FreePistonStirlingSingleNode;