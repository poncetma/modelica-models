model FPSE_Nonlinear_Majidniya_WithHeatTransfer
 "Implementation of Majidniya et al. (2019) Eqs. (15)/(16) for the RE-1000 case.
  ...
  parameter Real Vdot_max = abs(A_p*X_p*omega*sin(phi)/sin(atan((A_p*X_p*omega*sin(phi))/(A_p*X_p*omega*cos(phi)-(2*A_d-A_rod)*X_d*omega)))); //analytically solved max in closed form
  ...
  */ 
  ...
  parameter Real T_mesh_r1 = T_r1_nominal;
  parameter Real T_mesh_r2 = T_r2_nominal;
...
equation
  ...
  //Alt fixed values/placeholders in case no thermal coupling is desired (must also comment out heat balance equations). 
  /*
  T_h = T_h_nominal;
  T_r1 = T_r_nominal;
  T_r2 = T_r_nominal;
  T_k = T_k_nominal;
  T_c = T_r_nominal; //placeholder
  T_e = T_r_nominal; //placeholder
  */
  ...
  //p_c = m_total_gas*R_gas/V_avT;  
  //p_c = m_total_gas*R_gas*(A_p*C_c/T_k + A_d*C_e/T_h + V_h/T_h + V_k/T_k + V_r1/T_r1 + V_r2/T_r2);
  
  
  // Darcy pressure drops in the "bypass" region (heater, regenerator, cooler) - source of strong nonlinearity  
  /*
  ...
  */
  ...
  dP_r1 = 0.5 * rho_r1 * Cf_r * ((L_r/2)/d_h_r) * u_r * sqrt(u_r^2 + u_eps^2);
  dP_r2 = 0.5 * rho_r2 * Cf_r * ((L_r/2)/d_h_r) * u_r * sqrt(u_r^2 + u_eps^2); //still assume the same friction factor for now
  ...
  if Re_r1 < 60 then 
    Cf_r = 4*10^(1.73 - 0.93 * log10(Re_r1));    
  elseif Re_r1 < 1000 then
    Cf_r = 4*10^(0.714 - 0.365 * log10(Re_r1));
  else
    Cf_r = 4*10^(0.015 - 0.125 * log10(Re_r1));
  end if; 
  
  //Reynolds numbers      
  //--compute according to estimated max velocity
  /*
  Re_h = max(1000, rho_h*abs(u_h_max)*d_h_h/mu_h);
  Re_k = max(1000, rho_k*abs(u_k_max)*d_h_k/mu_k);
  //Re_r = max(1, rho_r*abs(u_r_max)*d_h_r/mu_r);
  Re_r1 = max(1000, rho_r1*abs(u_r_max)*d_h_r/mu_r1);
  Re_r2 = max(1000, rho_r2*abs(u_r_max)*d_h_r/mu_r2);
  */
  
  //--compute according to instantaneous local velocity--this approach seems cause damping to zero, for some reason.  
  Re_h = max(2001, rho_h*abs(u_h)*d_h_h/mu_h); 
  Re_k = max(2001, rho_k*abs(u_k)*d_h_k/mu_k);
  //Re_r = max(1001, rho_r*abs(u_r)*d_h_r/mu_r);
  Re_r1 = max(1001, rho_r1*abs(u_r)*d_h_r/mu_r1);
  Re_r2 = max(1001, rho_r2*abs(u_r)*d_h_r/mu_r2);
  ...
  F_load = B_load*v_p; //value from second Majidniya pub
  //F_load = C_load*v_p; //value from first Majidniya pub
  ...
  */
  ...
  HTC_r1 = Cf_r/4*(Re_r1*mu_r1*cp)/(2*d_h_r*Pr_r1);
  HTC_r2 = Cf_r/4*(Re_r2*mu_r2*cp)/(2*d_h_r*Pr_r2);
  ...
  //W_cp = p_c*der(Vc);
  //W_ed = p_e*der(Ve);
  //W_cp = p_c*(A_p*v_p - (A_d - A_rod)*v_d);
  //W_ed = p_e*A_d*v_d;
