...
import Modelica.Constants.pi;  
  ...
  final constant Integer N = 25 "Number of radial shells"; //tested up to several hundred nodes
  ...
  */
  ...
  Real q_gen_profile[N]; //Define q_gen based on a prescribed radial profile
...
equation
  ...
  for i in 2:N loop    
    q_flow[i] = -k_correlation(T[i]) * (T[i] - T[i-1]) / dr; //Note this only looks like a backward difference, but it's actually forward difference due to indexing shift
  end for;  
    
  // Outer boundary: heat flux based on half-delta-r to boundary and computed heat loss
  q_flow[N+1] = -k_correlation(T_outer_wall) * (T_outer_wall - T[N]) / (dr/2) + Q_loss/outer_wall_area; 
  ...
  rho_correlation(T[N]) * cp_correlation(T[N]) * der(T[N]) = q_flow[N]*(1/dr - 1/r_face[N] + 1/k_correlation(T[N])*(k_correlation(T_outer_wall)-k_correlation(T[N]))/(dr/2) ) - q_flow[N+1]/dr + q_gen[N]; // - Q_loss/V_shell[N];  //Could also subtract the heat loss as a volumetric sink
